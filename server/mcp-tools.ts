...
export function workspaceKeys(): string[] {
  return [...stores.keys()];
}

export function storeFor(workspaceRoot?: string): TeamStore {
...
export function withWorkspaceRoot<T extends Record<string, unknown>>(
  store: TeamStore,
  payload: T,
...
export function compactBoardForMcp<T extends { orders: Order[] }>(board: T): T {
  return {
    ...board,
...
export function activeOrdersForMcp<T extends { orders: Order[] }>(board: T): T {
  return {
    ...board,
...
export function joinPayloadForMcp(
  joined: unknown,
  board: { orders: Order[] },
  includeBoard?: boolean,
): Record<string, unknown> {
  const payload: Record<string, unknown> = { joined };
...
const workspaceRoot = z
  ...
  .optional()
  .describe(
    "Project root. Omit when possible: inferred from pwd if team/config.json exists, or the only workspace on the bus. Never ask the human to paste a path.",
  );
...
export function registerTeamTools(server: McpServer): void {
  server.tool(
    "team_start",
    "Lead one-shot: scaffold team/, ensure this chat joins as lead (default), optional seed worker order. Call this when the user wants the team running. Daemon must already be up (MCP connected) or pass ensureBus via CLI instead.",
    {
      workspaceRoot,
  ...
  server.tool(
    "team_scaffold",
    "Create team/ and knowledge/ files in a project. Does not require join. Use from /team-init.",
    { workspaceRoot },
  ...
  server.tool(
    "team_join",
    "Register this Cursor chat as lead or worker on the team bus. Call before other team tools.",
    {
      workspaceRoot,
      agentId,
    ...
    async (args) => {
      try {
        ...
        const agent = await store.join(args.agentId, args.role as Role, args.doing ?? "joined");
  ...
  server.tool(
    "team_whoami",
    "Return this agent's join record.",
  ...
  server.tool(
    "team_inbox_read",
    "Read the latest inbox messages.",
    {
      workspaceRoot,
  ...
  server.tool(
    ...
    "Broadcast a durable finding to the inbox (optionally pointing at a knowledge/ path).",
    {
      workspaceRoot,
      agentId,
    ...
    async (args) => {
      try {
        return jsonResult(
          await storeFor(args.workspaceRoot).publishFinding({
            from: args.agentId,
            title: args.title,
            body: args.body,
            path: args.path,
  ...
  server.tool(
    "team_claim_paths",
    "Lease file paths so other agents do not write them.",
    {
      workspaceRoot,
      agentId,
    ...
    async (args) => {
      try {
        return jsonResult(
          await storeFor(args.workspaceRoot).claimPaths(
            args.agentId,
            args.paths,
            args.orderId ?? null,
            args.ttlMs,
  ...
  server.tool(
    "team_release_paths",
    "Release path leases owned by this agent.",
    {
      workspaceRoot,
      agentId,
      paths: z.array(z.string()).min(1),
  ...
  server.tool(
    "team_suggest_worker",
    "Lead only. Rank workers for a delegate slice from claim paths and optional hint. Read-only; call before team_delegate.",
    {
      workspaceRoot,
      agentId,
  ...
  server.tool(
    "team_delegate",
    "Lead only. Create an order for a worker. Use parallel to keep working; assist for help; handoff to give the slice away.",
    {
      workspaceRoot,
      agentId,
  ...
  server.tool(
    "team_harvest",
    "Lead only. Return new done/blocked worker results since last harvest.",
    { workspaceRoot, agentId },
    async (args) => {
      try {
  ...
  server.tool(
    "team_cancel",
    "Lead only. Cancel an order.",
    { workspaceRoot, agentId, orderId: z.string().min(1) },
    async (args) => {
      try {
  ...
  server.tool(
    "team_nudge",
    "Lead only. Ping a worker via the inbox.",
    {
      workspaceRoot,
      agentId,
      to: z.string().min(1),
  ...
  server.tool(
    "team_poll",
    "Worker only. List open or claimed orders for this worker.",
    { workspaceRoot, agentId },
    async (args) => {
      try {
        const store = storeFor(args.workspaceRoot);
        ...
        await store.heartbeat(
          args.agentId,
          orders.length ? `polled, ${orders.length} order(s)` : "idle",
        );
        const profile = readProjectProfile(store.root);
  ...
  server.tool(
    "team_adapt",
    "Lead only. Build team/project.json from AGENTS.md, merged with optional overrides (chat beats file). Writes knowledge/project.md.",
    {
      workspaceRoot,
      agentId,
  ...
  server.tool(
    "team_read_project",
    "Read team/project.json profile (domain, worker personas). Lead uses before delegating; workers see persona on team_poll.",
    { workspaceRoot },
    async (args) => {
      try {
        const store = storeFor(args.workspaceRoot);
        ...
        return jsonResult(
          withWorkspaceRoot(store, {
            ...
            layering:
              "Existing project rules and user prompts stay authoritative; team protocol is coordination only.",
          }),
        );
      } catch (err) {
        return errorResult(err);
  ...
  server.tool(
    "team_claim_order",
    "Worker only. Claim an open order before working it.",
    { workspaceRoot, agentId, orderId: z.string().min(1) },
    async (args) => {
      try {
  ...
  server.tool(
    "team_report",
    "Worker only. Finish an order as done or blocked and write team/results/<id>.md.",
    {
      workspaceRoot,
      agentId,
    ...
    async (args) => {
      try {
        return jsonResult(
          await storeFor(args.workspaceRoot).report(args.agentId, args.orderId, args.status, args.body),
        );
      } catch (err) {
        return errorResult(err);
      }
  ...
  server.tool(
    "team_ask_lead",
    "Worker only. Block the order and ask the lead a question.",
    {
      workspaceRoot,
      agentId,
      orderId: z.string().min(1),
    ...
    async (args) => {
      try {
        return jsonResult(
          await storeFor(args.workspaceRoot).askLead(args.agentId, args.orderId, args.question),
        );
      } catch (err) {
        return errorResult(err);
      }
