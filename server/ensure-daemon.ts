...
export async function health(port: number): Promise<{ ok: boolean; body?: unknown }> {
  try {
...
export async function ensureDaemon(port = DEFAULT_PORT, timeoutMs = 4000): Promise<{
  ok: boolean;
  started: boolean;
  url: string;
  health?: unknown;
  ...
  if (existing.ok) {
    const running = existing.body as { buildId?: number } | undefined;
