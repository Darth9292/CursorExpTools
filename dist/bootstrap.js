...
export async function startTeam(input) {
    ...
    const agentId = input.agentId ?? (role === "lead" ? config.leadId : config.workers[0] ?? DEFAULT_WORKER_ID);
    let bus = {
    ...
    const joined = await store.join(agentId, role, input.doing ?? (role === "lead" ? "lead online, team started" : "worker online"));
    let seededOrderId = null;
    if (input.seedOrder && role === "lead") {
        const board = await store.getBoard();
        const open = board.orders.filter((o) => o.status === "open" || o.status === "claimed");
        if (open.length === 0) {
            const workers = config.workers.length > 0 ? config.workers : [DEFAULT_WORKER_ID];
