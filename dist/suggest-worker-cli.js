...
export function parseSuggestWorkerCliArgs(argv) {
    const args = { claim: [] };
...
export async function runSuggestWorkerCli(argv, cwd = process.cwd()) {
...
const isEntry = process.argv[1]?.replaceAll("\\", "/").endsWith("/suggest-worker-cli.js") ||
    process.argv[1]?.replaceAll("\\", "/").endsWith("/suggest-worker-cli.ts");
if (isEntry) {
    void main();
}
//# sourceMappingURL=suggest-worker-cli.js.map