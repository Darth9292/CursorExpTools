...
export function canonicalizeAgentId(raw: string): string {
  const s = raw.trim();
  if (!s) throw new Error("Agent id is empty. Use A-Z, then 1, 2, 3...");
...
export function nextWorkerId(taken: string[]): string {
...
export function takenIds(config: { leadId: string; workers: string[] }): string[] {
