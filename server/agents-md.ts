...
function parseSectionHeading(line: string): { id: string; headerTitle: string } | null {
...
const INLINE_WORKER =
  /^(?:[-*]\s+)?(?:\*\*)?(?:Agent\s+|Worker\s+)?([A-Za-z]|[1-9]\d*)(?:\*\*)?\s*[-–—:]\s*(.+)$/;

export function findAgentsMdPath(workspaceRoot: string): string | null {
...
export function readAgentsMd(workspaceRoot: string): string | null {
...
function extractPersonaFromBody(headerTitle: string, bodyLines: string[]): WorkerPersona {
...
export function parseAgentsMarkdown(
  ...
  const workers: Record<string, WorkerPersona> = {};
...
export function mergeWorkerPersonas(
  workerIds: string[],
  fromAgentsMd: Record<string, WorkerPersona>,
  chatOverrides: Record<string, WorkerPersona> | undefined,
  defaults: Record<string, WorkerPersona>,
): Record<string, WorkerPersona> {
...
export function normalizeOverrideWorkers(
  raw: Record<string, { title: string; focus: string }> | undefined,
): Record<string, WorkerPersona> | undefined {
  if (!raw) return undefined;
  const out: Record<string, WorkerPersona> = {};
  for (const [key, val] of Object.entries(raw)) {
    if (!val?.title || !val?.focus) continue;
